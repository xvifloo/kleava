 ...
 * query filtering across English and Bangla scripts with keyboard shortcuts.
 */
export const ChatSearch = forwardRef<HTMLInputElement, ChatSearchProps>(
  ({ value, onChange, onClear, placeholder = 'Search chats...', className }, ref) => {
    return (
      <div className={cn('relative w-full flex items-center select-none', className)}>
        ...
        <input
          ...
          className={cn(
            'w-full pl-8 pr-7 py-1.5 rounded-kleava-md font-ui',
        ...
        {value.trim().length > 0 && (
          <button
            ...
            className="absolute right-2 w-4 h-4 rounded-full flex items-center justify-center text-kleava-text-secondary hover:text-kleava-text-primary hover:bg-kleava-surface-soft transition-colors focus-ring-kleava"
          >
            <X className="w-3 h-3" />
          </button>
        )}
